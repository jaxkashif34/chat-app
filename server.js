...
socketIo.on('connection', (socket) => { // connection event is built-in event in socket.io and it will be triggered when a client connects to the server and it will pass the socket object to the callback function
  // when a client connects
  socket.on(CONSTANTS.JOIN_ROOM, ({ userName, roomName }) => { // listen to the joinRoom event and userName and roomName will receive when the client emits the joinRoom event
  ...
  socket.on(CONSTANTS.SEND_MSG, (msg) => {  // listen to the sendMsg event and msg will receive when the client emits the sendMsg event
  ...
  socket.on('disconnect', () => { // disconnect event is built-in event in socket.io and it will be triggered when a client disconnects from the server
